...
const SecondBrainSection = () => {
  const { title, subtitle, buttons, image } = typedContent.secondBrain;

  const titleRef = useGSAPAnimation({ type: 'textReveal', delay: 0.5 }) as React.RefObject<HTMLHeadingElement>;
  const descriptionRef = useGSAPAnimation({ type: 'textReveal', delay: 0.5 }) as React.RefObject<HTMLDivElement>;
  const buttonsRef = useGSAPAnimation({ type: 'slide', children: true, delay: 0.8 }) as React.RefObject<HTMLDivElement>;
    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 bg-[#FAFAFA] mt-[5%] rounded-2xl">
      <div className="py-10 md:py-20 flex flex-col justify-center items-center text-center">
        ...
        <div ref={descriptionRef} className="overflow-hidden">
          <p className="text-xl md:text-2xl text-gray-600 max-w-2xl mb-10">{subtitle}</p>
        </div>
        <div ref={buttonsRef} className="flex flex-row gap-3">
        {buttons?.map((button: { text: string; variant: 'primary' | 'secondary' }, index: number) => (
            <Button
              key={index}
              text={button.text}
